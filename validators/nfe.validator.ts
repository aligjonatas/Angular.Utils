...
export class NFeValidator {
  static validate(control: AbstractControl): ValidatorFn | { [key: string]: any } {
    let result = this.nfeValido(control.value);
    if (result?.Valid) {
      return null as any;
    }
  ...
  static nfeValido(accesskey: any): NFeValidator.ValidMessage | undefined {
    let validMessage: NFeValidator.ValidMessage = {} as NFeValidator.ValidMessage;
    ...
    // se os digitos 20 e 21 não formarem 55 e nem 65 (modelos de NFE), não é um NFE válido, retorna FALSE
    if (accesskey.substring(20, 22) != '55' && accesskey.substring(20, 22) != '65') {
      validMessage.Valid = false;
      validMessage.Message = 'Documento não é NF-e';
      return validMessage;
    }
    ...
    if (!NFeValidator.ValidateUFCode(codeParts[0])) {
    ...
    if (hasErrors) {
      ...
      var digit = NFeValidator.CheckDigit(accesskey.substring(0, accesskey.length - 1));
